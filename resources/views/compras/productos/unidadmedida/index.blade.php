...
<main class="h-full pb-16 overflow-y-auto">
    <div class="container grid px-6 mx-auto">
        ...
        <div class="flex justify-between items-center text-xs font-semibold tracking-wide text-gray-500 uppercase bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
            <div class="px-4 py-3">
                <center>
                <a href="{{ route('unidadmedida.create') }}">
                    <button type="submit" class="px-2 py-1 text-sm font-medium text-white transition-colors duration-200 rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1 dark:focus:ring-offset-darker">Nueva Unidad</button>
                </a>
                </center>
            </div>
            <div class="px-4 py-3">
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        ...
                        <th class="px-4 py-3 text-center">UNIDAD DE MEDIDA</th>
                    ...
                    @foreach($unidadmedida as $item)
                        ...
                        <td class="px-4 py-3 text-center text-sm">{{$item->IdUnidadMedida}}</td>
                        <td class="px-4 py-3 text-center text-sm">{{$item->NomUnidadMedida}}</td>
                        <td class="px-4 py-3 flex justify-center">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="{{ route('unidadmedida.edit', $item->IdUnidadMedida) }}">
                                    <button
                                ...
                                <a href="{{ route('unidadmedida.confirmar', $item->IdUnidadMedida) }}">
                                    <button
                                    class="flex items-
                ...
                <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
                    ...
                    <nav aria-label="Table navigation">
                        {{ $unidadmedida->links('pagination::tailwind') }}
                    </nav>
                    </span>
                </div>
            </div>

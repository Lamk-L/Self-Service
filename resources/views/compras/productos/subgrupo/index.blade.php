        ...
        <div class="flex justify-between items-center text-xs font-semibold tracking-wide text-gray-500 uppercase bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
            <div class="px-4 py-3">
                ...
                <a href="{{ route('subgrupo.create') }}">
                    <button type="submit" class="px-2 py-1 text-sm font-medium text-white transition-colors duration-200 rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1 dark:focus:ring-offset-darker">Nuevo SubGrupo</button>
                </a>
                </center>
            </div>
            <div class="px-4 py-3">
                ...
                <form role="search" method="GET">
                    <div class="relative w-full max-w-xl mr-6 focus-within:text-purple-500">
                        ...
                        <input class="w-full pl-10 pr-2 text-sm text-gray-700 placeholder-gray-600 bg-gray-100 border-0 rounded-md dark:placeholder-gray-500 dark:focus:shadow-outline-gray dark:focus:placeholder-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:placeholder-gray-500 focus:bg-white focus:border-purple-300 focus:outline-none focus:shadow-outline-purple form-input"
                            name="busqueda" type="search" placeholder="Buscar por Nombre" aria-label="Search" />
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        ...
                        <th class="px-4 py-3 text-center">SUBGRUPO</th>
                    ...
                    @foreach($subgrupos as $item)
                        ...
                        <td class="px-4 py-3 text-center text-sm">{{$item->IdSubGrupo}}</td>
                        <td class="px-4 py-3 text-center text-sm">{{$item->grupo->NomGrupo}}</td>
                        <td class="px-4 py-3 text-center text-sm">{{$item->NomSubGrupo}}</td>
                        <td class="px-4 py-3 flex justify-center">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="{{ route('subgrupo.edit', $item->IdSubGrupo) }}">
                                    <button
                                ...
                                <a href="{{ route('subgrupo.confirmar', $item->IdSubGrupo) }}">
                                    <button
                                    class="flex items-
                ...
                <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
                    ...
                    <nav aria-label="Table navigation">
                        {{ $subgrupos->links('pagination::tailwind') }}
                    </nav>
                    </span>
                </div>
            </div>

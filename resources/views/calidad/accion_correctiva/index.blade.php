@extends('welcome')
@section('titulo','Acción Correctiva')
        ...
        <div class="flex justify-between items-center text-xs font-semibold tracking-wide text-gray-500 uppercase bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
            ...
            <div class="px-4 py-3">
                <a href="{{ route('accioncorrectiva.create') }}">
                    <button type="submit" class="px-2 py-1 text-sm font-medium text-white transition-colors duration-200 rounded-md bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-1 dark:focus:ring-offset-darker">Nueva Acción</button>
                </a>
            </div>
            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        ...
                        <th class="px-4 py-3 text-center">NO CONFORMIDAD</th>
                        <th class="px-4 py-3 text-center">ACCIÓN CORRECTIVA</th>
                        <th class="px-4 py-3 text-center">RESULTADO</th>
                        <th class="px-4 py-3 text-center">ACCIONES</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    @foreach($AccionCorrectiva as $item)
                        <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-4 py-3 text-center text-sm">{{$item->IdAccionCorrectiva}}</td>
                        <td class="px-4 py-3 text-center text-sm">{{$item->NoConformidad->Descripcion}}</td>
                        <td class="px-4 py-3 text-center text-sm">{{$item->Descripcion}}</td>
                        <td class="px-4 py-3 text-center text-sm">{{$item->Resultado}}</td>
                        <td class="px-4 py-3 flex justify-center">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="{{ route('accioncorrectiva.edit', $item->IdAccionCorrectiva) }}">
                                    <button
                                ...
                                <a href="{{ route('accioncorrectiva.confirmar', $item->IdAccionCorrectiva) }}">
                                    <button
                                    class="flex items-
